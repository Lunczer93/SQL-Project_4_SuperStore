...
--4.l) The Quantity of Orders by Order Priority-- give it percetnage?? by scalar function
-- for which coountry the most, for  the most products as well (subcategory) using PIVOT for all years and using CTE
...
-- Sub Category
SELECT DISTINCT [YearOfOrderDate],[Sub-Category],[Order Priority], count(*) OVER (PARTITION BY [YearOfOrderDate],[Sub-Category],[Order Priority]) as [The Quantity of Orders]
FROM   [Project4].[dbo].[SuperStore]
ORDER BY [The Quantity of Orders] DESC
...
-- Shipping cost!! profit, sales etc year month quarter 
