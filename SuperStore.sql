...
DROP TABLE #SalesOfCountryByYear
CREATE TABLE #SalesOfCountryByYear
(
...
INSERT INTO #SalesOfCountryByYear
	SELECT DISTINCT [Country], 
	SUM([Sales]) OVER (PARTITION BY [Country]) as Sales,
	[YearOfOrderDate]
	FROM [Project4].[dbo].[SuperStore]


CREATE PROCEDURE SalesOfCountryByYear @Year int
AS;

	WITH GroupOfSalesOfCountryByYear AS (
	SELECT *, NTILE(3) OVER (ORDER BY Sales DESC) as Buckets
	FROM #SalesOfCountryByYear
	WHERE [YearOfOrderDate] = @Year
)
SELECT [Country], 
	CASE
		...
		WHEN [Buckets] = 3 THEN 'Low Sales'
	END AS [Level of Sales]
FROM GroupOfSalesOfCountryByYear

GO

EXEC SalesOfCountryByYear @Year = 2011
EXEC SalesOfCountryByYear @Year = 2012
EXEC SalesOfCountryByYear @Year = 2013
EXEC SalesOfCountryByYear @Year = 2014
