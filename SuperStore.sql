...
SELECT [Country], [City], [Customer Name]
FROM   [Project4].[dbo].[SuperStore]
...
SELECT DISTINCT 
			[Country], [City],[Sales],[Customer Name], 
			SUM([Sales]) OVER (PARTITION BY [Customer Name],[City]) as [Sales of City By Customer],
			SUM([Sales]) OVER (PARTITION BY [Customer Name],[Country]) as [Sales of Country By Customer]
	FROM   [Project4].[dbo].[SuperStore]
...
SELECT DISTINCT 
			[Country], [City],[Sales],[Customer Name], 
			SUM([Sales]) OVER (PARTITION BY [City],[Customer Name]) as [Sales of City By Customer],
			SUM([Sales]) OVER (PARTITION BY [Country],[Customer Name]) as [Sales of Country By Customer]
	FROM   [Project4].[dbo].[SuperStore]
	WHERE [Customer Name] = 'Brian Dahlen'

SELECT DISTINCT 
			[Country], [City], [Sales], [Customer Name], SUM([Sales]) OVER (PARTITION BY [Customer Name]) as [Sales Customer Name by City]
	FROM   [Project4].[dbo].[SuperStore]
	WHERE [City] = 'Herat'

SELECT TOP 10 WITH TIES [Country]
FROM [Project4].[dbo].[SuperStore]



SELECT DISTINCT 
			[Country], [City], [Customer Name], Sales,SUM([Sales]) OVER (PARTITION BY [City]) as [Sales Customer Name by City]
	FROM   [Project4].[dbo].[SuperStore]
	WHERE [Customer Name]  = 'Brian Dahlen'
...
--- Profit by City only for top 10 and bottom top and lowest top 10
-- WHICH CUSTOMER FROM WHICH CITY MAKE RANKING ETC!! FOR ALL CITY
-- TRY TO USE WINDOW FRAME AND LOOK FOR CUSTOMER FOR ONLY COUNTRY!!! WINDOWS FUNCTION. 
-- cHECK IF i CREATED A GOOD CODE IN PREVIOUS QUERIES!!
