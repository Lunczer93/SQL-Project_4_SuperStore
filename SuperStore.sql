...
-- Year Ranking, and in which year etc
SELECT DISTINCT 
[YearOfOrderDate],[Country], [City],[Sales],[Customer Name], 
...
SUM([Sales]) OVER (PARTITION BY [Customer Name],[City],[YearOfOrderDate]) as [Sales of City By Customer]
FROM   [Project4].[dbo].[SuperStore]
WHERE [YearOfOrderDate] = 2012 and [Customer Name] = 'Brian Dahlen'
...
-- COMBINE IT PROFIT AND SALES AND CITY YEAR BY YEAR !! 
-- do the same like Sales and CTE for only whichc makes profit and point out which ones not!!
